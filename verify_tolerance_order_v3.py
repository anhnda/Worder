"""
Synthetic verification of the tolerance-order certificate (Sections 3-5), v3.

v3 changes (on top of v2):

  (A) DIFFERENCE ESTIMATOR instead of product.  We measure
          s(rho) := (1/2) E[(y - y')^2] = sum_{j>=1} a_j (1 - rho^j)  +  sigma_obs^2
      The a_0 = (E g)^2 term CANCELS exactly (it does not appear in s), so the
      whole buggy/biased mean-estimation branch is GONE.  The only bias is the
      additive +sigma_obs^2, identical at every rho, which we subtract (known, or
      estimated from a rho=1 self-pair where (g(z)-g(z))^2/2 = 0 in expectation up
      to noise, isolating sigma_obs^2).  Constraints and the homogeneous objective
      h_K(a) = sum_{j>K} a_j - alpha * sum_{j>=1} a_j are written directly in terms
      of s(rho_l); T = sum_{j>=1} a_j enters only as a free LP variable pinned by
      the constraints, never measured through an unstable c(1)-a0 difference.

  (B) SHARED BASE MASK across overlaps.  One base batch z is drawn and queried
      ONCE; each rho_l reuses that same g(z) and only its coupled copy z'_l is a
      fresh query.  Pilot cost drops from 2*L*N0 to (L+1)*N0  (~halved), plus N0
      for the sigma^2 self-pair.

  (C) Everything else (LP feasibility certificate, honest Kmax=d, query accounting)
      is unchanged from v2.

----- v2 notes retained below -----
...
@torch.no_grad()
def _coupled_from_base(z, rho, gen, device):
    """Given a fixed base mask batch z (..,d) in {0,1}, produce a rho-coupled copy:
    retain each coord w.p. rho, resample Bernoulli(1/2) otherwise."""
    keep = torch.rand(z.shape, generator=gen, device=device) < rho
    fresh = torch.randint(0, 2, z.shape, generator=gen, device=device, dtype=PILOT_DTYPE)
    return torch.where(keep, z, fresh)
...
# ----------------------------------------------------------------------
def _build_polytope(s_hat, rhos, Kmax, eta_s):
    """A(eta) = { a = (a_1,...,a_Kmax) >= 0 :
        | sum_{j=1}^{Kmax} a_j (1 - rho_l^j) - s_hat_l | <= eta_s_l  for all l }.
    a_0 is absent by construction (the difference estimator cancels it), and T is a
    free quantity sum_{j>=1} a_j pinned only by these constraints.  Variables index
    degrees 1..Kmax, so column (j-1) corresponds to a_j.
    Returns (A_ub, b_ub, bounds, nvar) with nvar = Kmax."""
    nvar = Kmax
    rhos = np.asarray(rhos, dtype=LP_DTYPE)
    eta_s = np.broadcast_to(np.asarray(eta_s, dtype=LP_DTYPE), (len(rhos),))
    # W[l, j-1] = 1 - rho_l^j  for j=1..Kmax
    W = np.array([[1.0 - rho ** j for j in range(1, Kmax + 1)] for rho in rhos],
                 dtype=LP_DTYPE)  # (L, nvar)

    A_ub = np.vstack([W, -W])
    b_ub = np.concatenate([s_hat + eta_s, -(s_hat - eta_s)]).astype(LP_DTYPE)
    bounds = [(0, None)] * nvar
    return A_ub, b_ub, bounds, nvar


def certify_order(s_hat, rhos, Kmax, eta_s, alpha):
    """For each K in 1..Kmax-1, certify the SIGN of
        h_K(a) = sum_{j>K} a_j - alpha * sum_{j>=1} a_j
    over A(eta) (variables a_1..a_Kmax).  Returns dict K -> verdict in
    {SUFFICIENT, INSUFFICIENT, UNRESOLVED}, plus (h_lo, h_hi) arrays.
    Homogeneous objective; no division by any measured total (A)."""
    A_ub, b_ub, bounds, nvar = _build_polytope(s_hat, rhos, Kmax, eta_s)

    full = np.ones(nvar)                              # sum_{j>=1} a_j  (cols = a_1..a_Kmax)
    verdict = {}
    h_lo = np.full(Kmax + 1, np.nan)
    h_hi = np.full(Kmax + 1, np.nan)
    for K in range(1, Kmax + 1):                      # include K=Kmax (trivially sufficient)
        tail = np.zeros(nvar); tail[K:] = 1.0         # sum_{j>K} a_j; cols=a_1..a_Kmax (idx j-1)
        obj = tail - alpha * full                     # coefficients of h_K
...
def pilot_queries(L, N0):
    """Total model queries the pilot spends (v3, A+B):
       shared base z queried once (N0) + one coupled query per overlap (L*N0)
       + one extra base re-read for sigma^2 (N0).  => (L+2)*N0.
       (v2 spent 2*L*N0 + 3*N0.)"""
    return (L + 2) * N0
...
def exp_coverage(n_trials=200, d=49, N0=16000, sigma_obs=0.1, delta=0.1, seed=1):
    ...
    # true spectrum vector over degrees 1..Kmax for feasibility test
    a_star = np.zeros(Kmax)                      # cols index a_1..a_Kmax
    ...
    for t in range(n_trials):
        A_ub, b_ub, bounds, nvar = _build_polytope(s_hat[t], RHOS, Kmax, eta_s[t])
        # (#1 gate) does the TRUE spectrum satisfy A(eta)?  residual <= b_ub
        feas = np.all(A_ub @ a_star <= b_ub + 1e-9)
        feasible += feas

        verdict, h_lo, h_hi = certify_order(s_hat[t], RHOS, Kmax, eta_s[t], alpha)
        used += 1
...
def exp_budget_law(d=49, N0_grid=(2000, 4000, 8000, 16000, 32000),
    ...
    print(f"{'N0':>8} {'Q':>12} {'mean eta_s':>11} {'K=1 insuff':>11} {'false-suff':>11}")
    for N0 in N0_grid:
        s_hat, eta_s = run_pilot_batched(g, RHOS, N0, sigma_obs, n_trials,
        ...
        Q = pilot_queries(L, N0)
        print(f"{N0:>8} {Q:>12,} {eta_s.mean():>11.4f} "
